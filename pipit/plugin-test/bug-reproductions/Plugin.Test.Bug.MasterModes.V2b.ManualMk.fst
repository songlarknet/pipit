(* Hand-written equivalent of the core expression that the lifter emits
   ...
   that triggers it?" *)
module Plugin.Test.Bug.MasterModes.V2b.ManualMk
...
unfold let sel_mm : PPS.prim = PPS.Mkprim (Some "PipitRuntime.Prim.p'select") (PPT.FTFun sh_bool (PPT.FTFun sh_mm (PPT.FTFun sh_mm (PPT.FTVal sh_mm)))) (PRP.p'select)

unfold let bb : PPS.prim = PPS.Mkprim (Some "Prims.op_BarBar") (PPT.FTFun sh_bool (PPT.FTFun sh_bool (PPT.FTVal sh_bool))) (op_BarBar)

unfold let aa : PPS.prim = PPS.Mkprim (Some "Prims.op_AmpAmp") (PPT.FTFun sh_bool (PPT.FTFun sh_bool (PPT.FTVal sh_bool))) (op_AmpAmp)

unfold let eq_mode : PPS.prim = PPS.Mkprim (Some "Prims.op_Equality") (PPT.FTFun sh_mode (PPT.FTFun sh_mode (PPT.FTVal sh_bool))) (op_Equality)

unfold let eq_es : PPS.prim = PPS.Mkprim (Some "Prims.op_Equality") (PPT.FTFun sh_es (PPT.FTFun sh_es (PPT.FTVal sh_bool))) (op_Equality)

unfold let neq_es : PPS.prim = PPS.Mkprim (Some "Prims.op_disEquality") (PPT.FTFun sh_es (PPT.FTFun sh_es (PPT.FTVal sh_bool))) (op_disEquality)

unfold let eq_mm : PPS.prim = PPS.Mkprim (Some "Prims.op_Equality") (PPT.FTFun sh_mm (PPT.FTFun sh_mm (PPT.FTVal sh_bool))) (op_Equality)

unfold let eq_omi : PPS.prim = PPS.Mkprim (Some "Prims.op_Equality") (PPT.FTFun sh_omi (PPT.FTFun sh_omi (PPT.FTVal sh_bool))) (op_Equality)

unfold let is_some : PPS.prim = PPS.Mkprim (Some "FStar.Pervasives.Native.uu___is_Some") (PPT.FTFun sh_omi (PPT.FTVal sh_bool)) (Some?)
...
let mm_v2b_manual_core (cfg: config_int): eo_mm =
  ...
  let cond1 : ei_b =
    PXB.XApps (PXB.XApp (PXB.XApp (prm bb)
      (PXB.XApps (PXB.XApp (PXB.XApp (prm eq_mode) (bvar 4)) (vval Mode_Configure))))
      (PXB.XApps (PXB.XApp (PXB.XApp (prm eq_es) (bvar 3)) (vval S3_Severe))))
  in
  ...
  let cond_follower : ei_b =
    PXB.XApps (PXB.XApp (PXB.XApp (prm aa)
      (PXB.XApps (PXB.XApp (prm is_some) (bvar 2))))
  ...
  let cond_main : ei_b =
    PXB.XApps (PXB.XApp (PXB.XApp (prm aa)
      (PXB.XApps (PXB.XApp (PXB.XApp (prm aa)
        (PXB.XApps (PXB.XApp (prm is_some) (bvar 2))))
        (vval master_enable))))
      (PXB.XApps (PXB.XApp (PXB.XApp (prm neq_es) (bvar 3)) (vval S2_Error))))
  in
  // cond_eq_some : ref_msg = Some master_idx
  let cond_eq_some : ei_b =
    PXB.XApps (PXB.XApp (PXB.XApp (prm eq_omi) (bvar 2)) (vval (Some master_idx)))
  in
  ...
  let body_inner : ei_mm =
    PXB.XApps (PXB.XApp (PXB.XApp (PXB.XApp (prm sel_mm)
      cond1) (vval Master_Off)) body_pre0)
  in
