(* Hand-written equivalent of the core expression that the lifter emits
   ...
   that triggers it?" *)
module Plugin.Test.Bug.MasterModes.V2b.NoWrapNoSh
...
let ctx_outer : list PPS.shallow_type = [(PSSB.shallow (option master_index_int)); (PSSB.shallow error_severity); (PSSB.shallow mode)]
...
let ctx_inner : list PPS.shallow_type =
  (PSSB.shallow master_mode) :: (PSSB.shallow master_mode) :: ctx_outer

(* --- exp aliases at the two relevant contexts ----------------------- *)
unfold let eo_mm = PXB.exp PPS.table ctx_outer (PSSB.shallow master_mode)
unfold let ei_mm = PXB.exp PPS.table ctx_inner (PSSB.shallow master_mode)
unfold let ei_b  = PXB.exp PPS.table ctx_inner (PSSB.shallow bool)
...
unfold let sel_mm : PPS.prim = {
  ...
  prim_ty  = PPT.FTFun (PSSB.shallow bool) (PPT.FTFun (PSSB.shallow master_mode) (PPT.FTFun (PSSB.shallow master_mode) (PPT.FTVal (PSSB.shallow master_mode))));
  prim_sem = PRP.p'select;
}

unfold let bb : PPS.prim = {
  prim_id  = Some "Prims.op_BarBar";
  prim_ty  = PPT.FTFun (PSSB.shallow bool) (PPT.FTFun (PSSB.shallow bool) (PPT.FTVal (PSSB.shallow bool)));
  prim_sem = op_BarBar;
}
...
unfold let eq_mode : PPS.prim = {
  ...
  prim_ty  = PPT.FTFun (PSSB.shallow mode) (PPT.FTFun (PSSB.shallow mode) (PPT.FTVal (PSSB.shallow bool)));
  prim_sem = op_Equality;
}

unfold let eq_es : PPS.prim = {
  prim_id  = Some "Prims.op_Equality";
  prim_ty  = PPT.FTFun (PSSB.shallow error_severity) (PPT.FTFun (PSSB.shallow error_severity) (PPT.FTVal (PSSB.shallow bool)));
  prim_sem = op_Equality;
}
...
unfold let eq_mm : PPS.prim = {
  prim_id  = Some "Prims.op_Equality";
  prim_ty  = PPT.FTFun (PSSB.shallow master_mode) (PPT.FTFun (PSSB.shallow master_mode) (PPT.FTVal (PSSB.shallow bool)));
  prim_sem = op_Equality;
}

unfold let eq_omi : PPS.prim = {
  prim_id  = Some "Prims.op_Equality";
  prim_ty  = PPT.FTFun (PSSB.shallow (option master_index_int)) (PPT.FTFun (PSSB.shallow (option master_index_int)) (PPT.FTVal (PSSB.shallow bool)));
  prim_sem = op_Equality;
}

unfold let is_some : PPS.prim = {
  prim_id  = Some "FStar.Pervasives.Native.uu___is_Some";
  prim_ty  = PPT.FTFun (PSSB.shallow (option master_index_int)) (PPT.FTVal (PSSB.shallow bool));
  prim_sem = Some?;
}
...
let mm_v2b_manual_core (cfg: config_int): eo_mm =
  ...
  let mu_body : PXB.exp PPS.table ((PSSB.shallow master_mode) :: ctx_outer) (PSSB.shallow master_mode) =
