...
module AstOf  = Pipit.Source.Ast.OfFStar
...
let core_sigelt (e: env) (attrs: list Tac.term) (nm_src nm_core: option string) (m: mode) (tm: Tac.term): Tac.Tac (Tac.name & Tac.sigelt) =
  ...
  (* The plugin-emitted bindings (__core_*, __prim_*, __check_*, shallow
     ...
     [Pipit.Prim.HasStream.shallow] that have no C implementation, so
...
(*** Experimental: lift via `Pipit.Source.Ast` (v0) ***)

(* Build a `has_stream sty` instance term.
   - `Tv_Var n`: look up in `inst_map` (type-var-uniq → instance-binder-uniq),
     return a `Tv_Var` referencing the local instance binder.
   - `(a & b)` (i.e. `Tv_App (Tv_App (FVar tuple2) a) b`): build
     `has_stream_tup2 #a #b #(rec a) #(rec b)`.
...
(* New pipeline: parse the source binding into `Ast.ast`, then call
   `Pipit.Source.Ast.Lower.lower` to emit the core expression term.

   This mirrors `lift_tac` above but uses the AST-based path. It is
   intentionally minimal — only the subset of source programs that
   `Pipit.Source.Ast.OfFStar.lift_tm` knows how to translate will go
   through. *)
let lift_ast_tac (nm_src nm_core: string) : Tac.Tac (list Tac.sigelt) =
  ...
  let ctx_list_tm: Tac.term =
    Tac.fold_right
      (fun (b: AstOf.of_binder) (acc: Tac.term) ->
        let sh = AstLow.shallow_ty_for_env lower_env b.AstOf.ob_sty in
