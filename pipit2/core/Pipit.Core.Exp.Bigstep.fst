...
type bigstep (env: PES.sigenv): list row -> PES.term -> tuple -> Type =
  | BSPure:
      ...
      p: PP.pterm ->
      v: PR.value ->
      squash (PP.pterm_sem PP.val_empty p == Some v) ->
      bigstep env streams (PES.XPure p) [v]
  ...
  | BSVar:
      ...
      bigstep env (latest :: prefix) (PES.XBVar i) (L.index latest i)
  ...
  | BSTuple:
      ...
      es: list PES.term ->
      ws: list PR.value ->
      bigstep_widths env streams es ws ->
      bigstep env streams (PES.XTuple es) ws
  ...
  | BSNode:
      ...
      nd: PES.node ->
      v: list PR.value ->
      squash (L.assoc nm env.nodes == Some nd) ->
      bigstep env streams (PES.inst_node nd.params args nd.body) v ->
      bigstep env streams (PES.XNode nm args) v
  ...
  | BSProj:
      ...
      bigstep env streams e vs ->
      bigstep env streams (PES.XProj j e) [L.index vs j]
  ...
  | BSMu:
      ...
      body: PES.term ->
      v: list PR.value ->
      bigstep env streams (PES.subst_tuple (PES.XMu tys body) body) v ->
      bigstep env streams (PES.XMu tys body) v
  ...
  | BSLet:
      ...
      body: PES.term ->
      v: list PR.value ->
      bigstep env streams (PES.subst_tuple rhs body) v ->
      bigstep env streams (PES.XLet tys rhs body) v
  ...
  | BSContract:
      ...
      impl: PES.term ->
      v: list PR.value ->
      bigstep env streams impl v ->
      bigstep env streams (PES.XContract s rely guar impl) v
...
and bigstep_widths (env: PES.sigenv): list row -> list PES.term -> list PR.value -> Type =
  ...
  | BSWS:
      ...
      w: list PR.value ->
      ws: list PR.value ->
      bigstep env streams e w ->
      bigstep_widths env streams es ws ->
      bigstep_widths env streams (e :: es) (L.append w ws)

[@@no_auto_projectors]
noeq
type bigsteps (env: PES.sigenv): list row -> PES.term -> list (list PR.value) -> Type =
  | BSs0:
      e: PES.term ->
      bigsteps env [] e []
  | BSsS:
      ...
      r: row ->
      v: list PR.value ->
      bigsteps env rows e vs ->
      bigstep env (r :: rows) e v ->
      bigsteps env (r :: rows) e (v :: vs)

let bigstep_prop (env: PES.sigenv) (streams: list row) (e: PES.term) (v: list PR.value): prop =
  exists (h: bigstep env streams e v). True

let rec bigstep_det
  (#env: PES.sigenv) (#streams: list row) (#e: PES.term) (#v1 #v2: list PR.value)
  (h1: bigstep env streams e v1) (h2: bigstep env streams e v2)
  ...
  | BSNode _ nm _ nd1 _ _ hb1 ->
    let BSNode _ _ _ nd2 _ _ hb2 = h2 in
    assert (nd1 == nd2);
    bigstep_det hb1 hb2
...
and bigstep_widths_det
  ...
  (h1: bigstep_widths env streams es ws1) (h2: bigstep_widths env streams es ws2)
