...
let xsystem (c: context) (state: Type) (value: Type) = system (row c) state value
(* Same, but deterministic *)
let xdsystem (c: context) (state: Type) (value: Type) = dsystem (row c) state value
...
let rec dsystem_of_exp (e: exp 'c 'a { Causal.causal e }):
  ...
  | XBVar x -> dsystem_map (fun i -> CR.index 'c i x) dsystem_input
  | XVar x -> false_elim ()
  | XApp e1 e2 ->
  ...
  | XMu _ e1 ->
    ...
    dsystem_mu_causal #(row 'c) #('a & row 'c) (fun i v -> (v, i)) t
  | XLet b e1 e2 ->
...
let rec system_of_exp_rel (e: exp 'c 'a { Causal.causal e }):
  ...
  // else
    ...
    | XBVar x -> system_map (fun i -> CR.index 'c i x) system_input
    | XVar x -> false_elim ()
    | XApp e1 e2 ->
      assert_norm (Causal.causal (XApp e1 e2) ==> (Causal.causal e1 && Causal.causal e2));
